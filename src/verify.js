...
class Registration extends Component {
  ...
  submitButtonHandler = () => {
    if (
      ...
      const url =
        'https://cfu5pzbg5a.execute-api.eu-central-1.amazonaws.com/default/searchFace';
      const init = {
        headers: {
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.data.FaceMatches.length > 0) {
            alert(
              `yes you are ${response.data.FaceMatches[0].Face.ExternalImageId}`,
            );
          } else {
            alert('You are not valid user');
          }
        })
        .catch(e => console.log('error is here', e.response.data));
    }
  };
